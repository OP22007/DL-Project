...
const FutureChart = ({ symbol, data }) => {
  ...
  const sendDataToBackend = async (lowPrices: number[]) => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:5000/predict_next",
        {
            ticker: symbol,
            features: Array.isArray(data) ? data.map((item) => item.low) : [],
            type: "low",
  ...
  // console.log(data)
  useEffect(() => {
    const lowPrices = data.map((entry) => entry.low).slice(-60);
    // console.log(lowPrices)
    sendDataToBackend(lowPrices);
  }, [data,timeRange])
