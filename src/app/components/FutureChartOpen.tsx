...
const FutureChart = ({ symbol, data }) => {
  ...
  const sendDataToBackend = async (openPrices: number[]) => {
    try {
      const response = await axios.post(
        "http://127.0.0.1:5000/predict_next",
        {
            ticker: symbol,
            features: Array.isArray(data) ? data.map((item) => item.open) : [],
            type: "open",
  ...
  // console.log(data)
  useEffect(() => {
    const openPrices = data.map((entry) => entry.open).slice(-60);
    // console.log(lowPrices)
    sendDataToBackend(openPrices);
  }, [data,timeRange])
